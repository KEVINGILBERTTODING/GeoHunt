...
class SinglePlayerVm @Inject constructor(
    ...
    val _trueLocation = MutableStateFlow(
        getRandomCityLatLngUseCase(cityRepository.getAllCity().first()))
    ...
    private val _selectedCity = MutableStateFlow(cityRepository.getAllCity().first())
    ...
    fun getPhotos() {
        viewModelScope.launch {
            ...
            if (responseSinglePhotos is Resource.Success) {
                ...
                _loadingState.emit(Resource.Error("Unknown error", Exception()))
            }
        }
    }
